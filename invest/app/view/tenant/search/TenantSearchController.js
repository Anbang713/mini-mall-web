 ...
 */
Ext.define('invest.view.tenant.search.TenantSearchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.invest.tenant.search',
        ...
        var me = this,
            batchprocessor = Ext.create({
                ...
                executeFn: function (record, callback) {
                    if (action == 'enable') {
                        invest.view.tenant.TenantService.doEnable(record.getData(), false).then(function (response) {
                            callback.onSuccess(response);
                        }).otherwise(function (response) {
                            callback.onFailure(response);
                        });
                    } else if (action == 'disable') {
                        invest.view.tenant.TenantService.doDisable(record.getData(), false).then(function (response) {
                            callback.onSuccess(response);
                        }).otherwise(function (response) {
                            callback.onFailure(response);
                        });
    ...
    doItemAction: function (action, actionText, record) {
        ...
        if (action === 'edit') {
            ...
            invest.view.tenant.TenantService[action](entity).then(function () {
                me.getView().down('#searchGrid').doSearch();
            });
        }
    },
