 ...
 */
Ext.define('invest.view.brand.search.BrandSearchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.invest.brand.search',
        ...
        var me = this,
            batchprocessor = Ext.create({
                ...
                executeFn: function (record, callback) {
                    if (action == 'enable') {
                        invest.view.brand.BrandService.doEnable(record.getData(), false).then(function (response) {
                            callback.onSuccess(response);
                        }).otherwise(function (response) {
                            callback.onFailure(response);
                        });
                    } else if (action == 'disable') {
                        invest.view.brand.BrandService.doDisable(record.getData(), false).then(function (response) {
                            callback.onSuccess(response);
                        }).otherwise(function (response) {
                            callback.onFailure(response);
                        });
    ...
    doItemAction: function (action, actionText, record) {
        ...
        if (action === 'edit') {
            ...
            invest.view.brand.BrandService[action](entity).then(function () {
                me.getView().down('#searchGrid').doSearch();
            });
        }
    },
