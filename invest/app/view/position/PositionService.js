 ...
 */
Ext.define('invest.view.position.PositionService', {
    alias: 'invest.position.service',
    alternateClassName: 'PositionService',
    requires: [
        'Cxt.util.Toast',
        'Cxt.util.Window',
        'Ext.util.Format'
    ...
    save: function (entity) {
        ...
        Ext.Ajax.request({
            url: invest.view.position.Position.servicePath,
            failureToast: true,
            waitMsg: '正在保存...',
            jsonData: entity
        ...
        Ext.Msg.confirm("提示", "确定要停用" + Ext.util.Format.ucn(entity) + "吗?", function (optional) {
            if (optional == 'yes') {
                me.doDisable(entity, true).then(function () {
                    Cxt.util.Toast.success('铺位(' + Ext.util.Format.ucn(entity) + ')' + '停用成功');
                    deferred.resolve();
    ...
    doDisable: function (entity, isView) {
        var deferred = Ext.create('Ext.Deferred');
        var ajax = {
            url: invest.view.position.Position.servicePath + entity['uuid'],
            method: 'PUT',
            params: {
        ...
        Ext.Msg.confirm("提示", "确定要启用" + Ext.util.Format.ucn(entity) + "吗?", function (optional) {
            if (optional == 'yes') {
                me.doEnable(entity, true).then(function () {
                    Cxt.util.Toast.success('铺位(' + Ext.util.Format.ucn(entity) + ')' + '启用成功');
                    deferred.resolve();
                }).otherwise(function (response) {
                    deferred.reject(response);
                });
    ...
    load: function (urlParams) {
        ...
        if (!urlParams || !urlParams.uuid) {
            ...
            Ext.Ajax.request({
                url: invest.view.position.Position.servicePath + urlParams.uuid,
                failureToast: true,
                method: 'GET',
                waitMsg: '正在加载...'
            }).then(function (response) {
