 ...
 */
Ext.define('invest.view.position.search.PositionSearchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.invest.position.search',
        ...
        var me = this,
            batchprocessor = Ext.create({
                ...
                executeFn: function (record, callback) {
                    if (action == 'enable') {
                        invest.view.position.PositionService.doEnable(record.getData(), false).then(function (response) {
                            callback.onSuccess(response);
                        }).otherwise(function (response) {
                            callback.onFailure(response);
                        });
                    } else if (action == 'disable') {
                        invest.view.position.PositionService.doDisable(record.getData(), false).then(function (response) {
                            callback.onSuccess(response);
                        }).otherwise(function (response) {
                            callback.onFailure(response);
                        });
    ...
    doItemAction: function (action, actionText, record) {
        ...
        if (action === 'edit') {
            ...
            invest.view.position.PositionService[action](entity).then(function () {
                me.getView().down('#searchGrid').doSearch();
            });
        }
    },
