 ...
 */
Ext.define('invest.view.floor.FloorService', {
    alias: 'invest.floor.service',
    alternateClassName: 'FloorService',
    requires: [
        'Cxt.util.Toast',
        'Cxt.util.Window',
        'Ext.util.Format'
    ...
    save: function (entity) {
        ...
        Ext.Ajax.request({
            url: invest.view.floor.Floor.servicePath,
            failureToast: true,
            waitMsg: '正在保存...',
            jsonData: entity
        ...
        Ext.Msg.confirm("提示", "确定要停用" + Ext.util.Format.ucn(entity) + "吗?", function (optional) {
            if (optional == 'yes') {
                me.doDisable(entity, true).then(function () {
                    Cxt.util.Toast.success('楼层(' + Ext.util.Format.ucn(entity) + ')' + '停用成功');
                    deferred.resolve();
    ...
    doDisable: function (entity, isView) {
        var deferred = Ext.create('Ext.Deferred');
        var ajax = {
            url: invest.view.floor.Floor.servicePath + entity['uuid'],
            method: 'PUT',
            params: {
        ...
        Ext.Msg.confirm("提示", "确定要启用" + Ext.util.Format.ucn(entity) + "吗?", function (optional) {
            if (optional == 'yes') {
                me.doEnable(entity, true).then(function () {
                    Cxt.util.Toast.success('楼层(' + Ext.util.Format.ucn(entity) + ')' + '启用成功');
                    deferred.resolve();
                }).otherwise(function (response) {
                    deferred.reject(response);
                });
    ...
    load: function (urlParams) {
        ...
        if (!urlParams || !urlParams.uuid) {
            ...
            Ext.Ajax.request({
                url: invest.view.floor.Floor.servicePath + urlParams.uuid,
                failureToast: true,
                method: 'GET',
                waitMsg: '正在加载...'
            }).then(function (response) {
