/**时间控件选择面板
 * Created by ningcheng on 2016/6/14.
 */
Ext.define('Cxt.picker.DateTime', {
    ...
    renderTpl: [
        ...
        '<div id="{id}-footerEl" role="presentation" style="background-color:#f5f5f5;border-top:0px solid #99BCE8;width:100%">{%this.renderHour(values, out)%}{%this.renderMinute(values, out)%}{%this.renderSecond(values, out)%}<center>{%this.renderOkQueDingBtn(values, out)%}&nbsp;&nbsp;{%this.renderTodayBtn(values, out)%}</center></div>',
        '</tpl>',
    ...
    getRefOwner: function () {
        return this.pickerField || this.callParent(arguments);
    },
        ...
        var spinnerCfg = Ext.apply({}, me.defaults, {
            ...
            listeners: {
                change: function (field, value) {
                    field.setValue((value > field.maxNum) ? field.minNum : field.value);
                    field.setValue((value < field.minNum) ? field.maxNum : field.value);
                }
            }
        });

        me.hour = Ext.create('Ext.form.field.Number', Ext.apply({}, spinnerCfg, {
            fieldLabel: '时',
            minNum: 0,
            maxNum: 23
        }));

        me.minute = Ext.create('Ext.form.field.Number', Ext.apply({}, spinnerCfg, {
            fieldLabel: '分',
            minNum: 0,
            maxNum: 59
        }));

        me.second = Ext.create('Ext.form.field.Number', Ext.apply({}, spinnerCfg, {
            fieldLabel: '秒',
            minNum: 0,
            maxNum: 59
        }));
    ...
    getSelectedDate: function (date) {
        var me = this,
            t = Ext.Date.clearTime(date).getTime(),
            cells = me.cells,
            cls = me.selectedCls,
    ...
    handleDateClick: function (e, t) {
        ...
        if (!me.disabled && t.dateValue && !Ext.fly(t.parentNode).hasCls(me.disabledCellCls)) {
            ...
            // event.
            me.selectedUpdate(Ext.Date.clearTime(new Date(t.dateValue)));
            me.onSelect();
        }
    },
    ...
    selectToday: function () {
        ...
        if (btn && !btn.disabled) {
            me.hour.setValue(curDate.getHours());
            me.minute.setValue(curDate.getMinutes());
            me.second.setValue(curDate.getSeconds());
            me.setValue(curDate);
            ...
            //    handler.call(me.scope || me, me, me.value);
            //}
            me.selectedUpdate(Ext.Date.clearTime(curDate));
            me.onSelect();
        }
        return me;
    },
    ...
    update: function (date, forceRefresh) {
        ...
        //添加时间相关
        date.setHours(me.hour ? me.hour.getValue() : null);
        date.setMinutes(me.minute ? me.minute.getValue() : null);
        date.setSeconds(me.second ? me.second.getValue() : null);
